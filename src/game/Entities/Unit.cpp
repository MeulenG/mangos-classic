...
void Unit::RemoveSpellsCausingAura(AuraType auraType, SpellAuraHolder* except, bool onlyMechanic)
{
    for (AuraList::const_iterator iter = m_modAuras[auraType].begin(); iter != m_modAuras[auraType].end();)
    {
        if ((*iter)->GetHolder() == except || (onlyMechanic && GetAllSpellMechanicMask((*iter)->GetSpellProto()) == 0))
        {
            ++iter;
            continue;
        }
...
void Unit::DealDamageMods(Unit* dealer, Unit* victim, uint32& damage, uint32* absorb, DamageEffectType damagetype, SpellEntry const* spellInfo)
{
    ...
    if (dealer) // dealer is optional
    {
        if (UnitAI* ai = dealer->AI()) // Script Event damage Deal
            ai->DamageDeal(victim, damage, damagetype, spellInfo);
    }
...
{
    SendEnvironmentalDamageLog(DAMAGE_FALL, this->GetHealth(), 0, 0);
    DealDamage(this, this, this->GetHealth(), nullptr, SELF_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, nullptr, false);
}
...
uint32 Unit::DealDamage(Unit* dealer, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellInfo, bool durabilityLoss, Spell* spell)
{
        ...
        || damagetype == SPLIT_DAMAGE))
    {
        ...
        {
            if (spellInfo && spellInfo->Id == aura.second->GetId()) // Not drop auras added by self
                continue;
            ...
            // for creatures, spells which started travelling sooner than aura application should not be broken - we assume same for players
            if (spell)
                if (spell->GetSpellStartTravelling() <= aura.second->GetAuraApplyMSTime())
                    continue;
        ...
        if (Spell* spell = victim->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
            if (spell->m_spellInfo->ChannelInterruptFlags & actionInterruptFlags)
                victim->InterruptSpell(CURRENT_CHANNELED_SPELL);
    }
...
void Unit::Kill(Unit* killer, Unit* victim, DamageEffectType damagetype, SpellEntry const* spellInfo, bool durabilityLoss, bool duel_hasEnded)
{
    ...
    */
    // call kill spellInfo proc event (before real die and combat stop to triggering auras removed at death/combat stop)
    ...
    */
    if (victim->IsPlayer())          // Killed player
    {
        ...
        {
            if (!spellInfo || !spellInfo->HasAttribute(SPELL_ATTR_EX3_NO_DURABILITY_LOSS))
            {
...
void Unit::HandleDamageDealt(Unit* dealer, Unit* victim, uint32& damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellInfo, bool duel_hasEnded)
{
    ...
    if (dealer)
        dealer->InterruptOrDelaySpell(victim, damagetype, spellInfo);
...
void Unit::InterruptOrDelaySpell(Unit* pVictim, DamageEffectType damagetype, SpellEntry const* spellInfo)
{
    bool dotDamage = (damagetype == DOT || spellInfo && spellInfo->HasAttribute(SPELL_ATTR_EX3_TREAT_AS_PERIODIC));
    if (damagetype == NODAMAGE || damagetype == SPELL_DAMAGE_SHIELD || damagetype == SPLIT_DAMAGE)
        return;
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
    {
        if (Spell* spell = pVictim->GetCurrentSpell(CurrentSpellTypes(i)))
        {
            ...
            {
                spell = pVictim->GetCurrentSpell(CURRENT_CHANNELED_SPELL); // fetch again because spellInfo couldve been interrupted before
                if (spell)
                {
...
void Unit::JustKilledCreature(Unit* killer, Creature* victim, Player* responsiblePlayer)
{
    ...
    if (killer && victim->GetCreatureType() == CREATURE_TYPE_CRITTER && killer->GetTypeId() == TYPEID_PLAYER)
    {
        if (CreatureInfo const* normalInfo = ObjectMgr::GetCreatureTemplate(victim->GetEntry()))
            static_cast<Player*>(killer)->KilledMonster(normalInfo, victim);
    }

    // Interrupt channeling spellInfo when a Possessed Summoned is killed
    SpellEntry const* spellInfo = sSpellTemplate.LookupEntry<SpellEntry>(victim->GetUInt32Value(UNIT_CREATED_BY_SPELL));
    if (spellInfo && spellInfo->HasAttribute(SPELL_ATTR_EX_TOGGLE_FARSIGHT) && spellInfo->HasAttribute(SPELL_ATTR_EX_IS_CHANNELED))
    {
...
SpellCastResult Unit::CastSpell(SpellCastArgs& args, uint32 spellId, uint32 triggeredFlags, Item* castItem, Aura* triggeredByAura, ObjectGuid originalCaster, SpellEntry const* triggeredBy)
{
    SpellEntry const* spellInfo = sSpellTemplate.LookupEntry<SpellEntry>(spellId);
    ...
    return CastSpell(args, spellInfo, triggeredFlags, castItem, triggeredByAura, originalCaster, triggeredBy);
}

SpellCastResult Unit::CastSpell(SpellCastArgs& args, uint32 spellId, TriggerCastFlags triggeredFlags, Item* castItem, Aura* triggeredByAura, ObjectGuid originalCaster, SpellEntry const* triggeredBy)
{
    return CastSpell(args, spellId, uint32(triggeredFlags), castItem, triggeredByAura, originalCaster, triggeredBy);
}

SpellCastResult Unit::CastSpell(SpellCastArgs& args, SpellEntry const* spellInfo, TriggerCastFlags triggeredFlags, Item* castItem, Aura* triggeredByAura, ObjectGuid originalCaster, SpellEntry const* triggeredBy)
{
    return CastSpell(args, spellInfo, uint32(triggeredFlags), castItem, triggeredByAura, originalCaster, triggeredBy);
}

SpellCastResult Unit::CastSpell(SpellCastArgs& args, SpellEntry const* spellInfo, uint32 triggeredFlags, Item* castItem, Aura* triggeredByAura, ObjectGuid originalCaster, SpellEntry const* triggeredBy)
{
    if (!spellInfo)
    {
        if (triggeredByAura)
    ...
    if (Unit* target = args.GetTarget())
        targets.setUnitTarget(target);

    if (args.IsScriptValueSet())
        spell->SetScriptValue(args.GetScriptValue());

    if (int32* bp = args.GetBasePoints(EFFECT_INDEX_0))
        spell->m_currentBasePoints[EFFECT_INDEX_0] = *bp;

    if (int32* bp = args.GetBasePoints(EFFECT_INDEX_1))
        spell->m_currentBasePoints[EFFECT_INDEX_1] = *bp;

    if (int32* bp = args.GetBasePoints(EFFECT_INDEX_2))
        spell->m_currentBasePoints[EFFECT_INDEX_2] = *bp;
...
void Unit::DealSpellDamage(Unit* affectiveCaster, SpellNonMeleeDamage* spellDamageInfo, bool durabilityLoss, bool resetLeash)
{
    ...
    DealDamage(affectiveCaster, victim, spellDamageInfo->damage, &cleanDamage, resetLeash ? SPELL_DIRECT_DAMAGE : SPELL_DAMAGE_SHIELD, GetSchoolMask(spellDamageInfo->school), spellProto, durabilityLoss, spellDamageInfo->spell);
}
...
void Unit::CalculateMeleeDamage(Unit* pVictim, CalcDamageInfo* calcDamageInfo, WeaponAttackType attackType /*= BASE_ATTACK*/)
{
    ...
    {
        case BASE_ATTACK:
            calcDamageInfo->procAttacker = PROC_FLAG_DEAL_MELEE_SWING | PROC_FLAG_MAIN_HAND_WEAPON_SWING;
...
void Unit::DealMeleeDamage(CalcDamageInfo* calcDamageInfo, bool durabilityLoss)
{
    ...
    CleanDamage cleanDamage(calcDamageInfo->cleanDamage, calcDamageInfo->attackType, calcDamageInfo->hitOutCome, calcDamageInfo->totalDamage || calcDamageInfo->absorb);
    ...
    if (calcDamageInfo->totalDamage)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            for (SpellAuraHolderMap::const_iterator itr = vAuras.begin(); itr != vAuras.end(); ++itr)
            {
                ...
                if (spellInfo->HasAttribute(SPELL_ATTR_EX3_ALWAYS_HIT) && spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN && ((*itr).second->GetCasterGuid() == GetObjectGuid()))
        ...
        for (AuraList::const_iterator i = vDamageShields.begin(); i != vDamageShields.end();)
        {
            if (alreadyDone.find(*i) == alreadyDone.end())
            {
                ...
                // uint32 absorb;
                // uint32 resist;
                // CalcAbsorbResist(victim, SpellSchools(spellInfo->School), SPELL_DIRECT_DAMAGE, damage, &absorb, &resist);
                ...
                DealDamage(victim, this, damage, nullptr, SPELL_DAMAGE_SHIELD, GetSpellSchoolMask(spellProto), spellProto, true);
...
void Unit::CalculateDamageAbsorbAndResist(Unit* caster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32* absorb, int32* resist, bool canReflect, bool canResist, bool binary)
{
    ...
    // Reflect damage spells (not cast any damage spellInfo in aura lookup)
    ...
    */
    ...
    // Cast back reflect damage spellInfo if caster exists
    ...
    if (caster && caster != this)
    {
        ...
        for (AuraList::const_iterator i = vSplitDamageFlat.begin(), next; i != vSplitDamageFlat.end() && remainingDamage >= 0; i = next)
        {
            ...
            {
                Unit::SendSpellMiss(this, caster, (*i)->GetSpellProto()->Id, SPELL_MISS_IMMUNE);
                return;
            }
...
void Unit::CalculateAbsorbResistBlock(Unit* caster, SpellNonMeleeDamage* spellDamageInfo, SpellEntry const* spellInfo, WeaponAttackType attType)
{
    ...
    CalculateDamageAbsorbAndResist(caster, GetSchoolMask(spellDamageInfo->school), SPELL_DIRECT_DAMAGE, spellDamageInfo->damage, &spellDamageInfo->absorb, &spellDamageInfo->resist, IsReflectableSpell(spellInfo), IsResistableSpell(spellInfo), IsBinarySpell(*spellInfo));
...
void Unit::SendMeleeAttackStart(const Unit& victim) const
{
...
void Unit::SendMeleeAttackStop(const Unit& victim) const
{
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* pVictim, SpellEntry const* spellInfo, SpellSchoolMask schoolMask, uint32* heartbeatResistChance/* = nullptr*/)
{
    ...
    if (victim->CanDeflectAbility(this, spellInfo))
       die.set(UNIT_COMBAT_DIE_DEFLECT, victim->CalculateAbilityDeflectChance(this, spellInfo));
    */
...
SpellMissInfo Unit::SpellHitResult(WorldObject* caster, Unit* pVictim, SpellEntry const* spellInfo, uint8 effectMask, bool reflectable, bool reflected, uint32* heartbeatResistChance/* = nullptr*/)
{
    ...
    if (!ignoreRestrictions && IsPositiveEffectMask(spellInfo, effectMask, caster, pVictim))
    {
        ...
        // Check for immune to damage as hit result if spellInfo hit composed entirely out of damage effects
        if (IsSpellEffectsDamage(*spellInfo, effectMask) && pVictim->IsImmuneToDamage(schoolMask))
            return SPELL_MISS_IMMUNE;

        return SPELL_MISS_NONE;
    }
    // !!!UNHACKME: Self-resists suppression hack for channeled spells until spellInfo execution is modernized with effectmasks: Drain Soul, Blizzard, RoF
...
bool Unit::CanCrushInCombat() const
{
    if (!CanCrush())
        return false;
    if (GetTypeId() == TYPEID_UNIT && ((Creature*)this)->GetSettings().HasFlag(CreatureStaticFlags2::NO_CRUSHING_BLOWS))
        return false;
    return true;
}
...
bool Unit::CanBlockAbility(const Unit* attacker, const SpellEntry* ability, bool miss) const
{
    ...
    // Spells with an attribute must be rolled for block once on spellInfo hit die
...
float Unit::GetCritChance(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
        // Base crit chance (needed for units only, already included for players)
        chance += 5.0f;
    // Pick highest spellInfo crit available for given school mask
...
float Unit::GetCritChance(const SpellEntry* entry, SpellSchoolMask schoolMask) const
{
    ...
    if (entry->HasAttribute(SPELL_ATTR_EX3_NORMAL_RANGED_ATTACK)) // Wand shoot forced to use ranged crit
...
float Unit::GetMissChance(SpellSchoolMask /*schoolMask*/) const
{
    ...
    if (GetTypeId() == TYPEID_UNIT && ((const Creature*)this)->IsTotem())
        return 0.0f;
    // Default base chance to be missed by a spellInfo for all acting units: 4%
    float chance = 4.0f;
    return chance;
}
...
float Unit::GetMissChance(const SpellEntry* entry, SpellSchoolMask schoolMask) const
{
    ...
    if (entry->HasAttribute(SPELL_ATTR_EX3_NORMAL_RANGED_ATTACK))
    ...
    else
    {
        switch (entry->DmgClass)
        {
            ...
            case SPELL_DAMAGE_CLASS_RANGED:
                chance += GetMissChance(GetWeaponAttackType(entry));
                break;
                // By default, we use spellInfo miss chance even for spells without dmgclass
            default:
...
float Unit::CalculateSpellCritChance(const Unit* victim, SpellSchoolMask schoolMask, const SpellEntry* spellInfo) const
{
    ...
    // wands are spellInfo attacks but qualify for ranged crit calculation
    ...
    if (!IsPositiveSpell(spellInfo, this, victim))
        chance += victim->GetCritTakenChance(schoolMask, SpellDmgClass(spellInfo->DmgClass));
    for (auto i : GetScriptedLocationAuras(SCRIPT_LOCATION_CRIT_CHANCE))
    {
...
{
    if (!spell || spell->HasAttribute(SPELL_ATTR_EX3_ALWAYS_HIT))
        return 0.0f;
...
float Unit::CalculateSpellResistChance(const Unit* victim, SpellSchoolMask schoolMask, const SpellEntry* spell) const
{
    ...
    // Chance to fully resist a spellInfo by magic resistance
    // TODO: check if binaries are affected by SPELL_ATTR_EX3_ALWAYS_HIT
    ...
    // Chance to fully resist entire spellInfo by it's dispel type
    ...
    // Chance to fully resist entire spellInfo by it's mechanic
...
void Unit::SetCurrentCastedSpell(Spell* newSpell)
{
    ...
    // break same type spellInfo if it is not delayed
    ...
    switch (CSpellType)
    {
        case CURRENT_GENERIC_SPELL:
        {
            ...
            {
                if (!channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING) && !newSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING))
        ...
        case CURRENT_CHANNELED_SPELL:
        {
            ...
            // Channeled spellInfo can only be one
            // Spells with this attribute can be cast in parallel with generic spells
            if (!newSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING) && !newSpell->m_IsTriggeredSpell)
            {
                if (Spell const* genericSpell = m_currentSpells[CURRENT_AUTOREPEAT_SPELL])
                    if (!genericSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING))
        ...
        case CURRENT_AUTOREPEAT_SPELL:
        {
            ...
            if (newSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX3_CASTING_CANCELS_AUTOREPEAT))
            {
                ...
                {
                    if (!newSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING))
                        InterruptSpell(CURRENT_CHANNELED_SPELL, false);
                }
            }
        ...
        default:
        {
            // other spellInfo types don't break anything now
        } break;
    }

    // current spellInfo (if it is still here) may be safely deleted now
    if (m_currentSpells[CSpellType])
        m_currentSpells[CSpellType]->SetReferencedFromCurrent(false);

    // set new current spellInfo
...
void Unit::InterruptSpell(CurrentSpellTypes spellType, bool withDelayed)
{
    if (m_currentSpells[spellType] && (withDelayed || m_currentSpells[spellType]->getState() != SPELL_STATE_TRAVELING))
    {
        ...
        if (m_currentSpells[spellType]->CanBeInterrupted())
        {
            ...
            // cancel can interrupt spellInfo already (caster cancel ->target aura remove -> caster iterrupt)
...
{
    // We don't do loop here to explicitly show that melee spellInfo is excluded.
    ...
    if (Spell const* genericSpell = m_currentSpells[CURRENT_GENERIC_SPELL])
    {
        if (genericSpell->getState() != SPELL_STATE_FINISHED)
        {
            ...
            bool isAutoNonInterrupting = forAutoIgnore && genericSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX2_DO_NOT_RESET_COMBAT_TIMERS);
    ...
    if (!skipChanneled)
    {
        if (Spell const* channeledSpell = m_currentSpells[CURRENT_CHANNELED_SPELL])
        {
            ...
            if (!forMovement)
                attributeResult = channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX4_ALLOW_CAST_WHILE_CASTING);

            bool isAutoNonInterrupting = forAutoIgnore && channeledSpell->m_spellInfo->HasAttribute(SPELL_ATTR_EX2_DO_NOT_RESET_COMBAT_TIMERS);
...
bool Unit::AddSpellAuraHolder(SpellAuraHolder* holder)
{
    ...
    // ghost spellInfo check, allow apply any auras at player loading in ghost mode (will be cleanup after load)
    if (!IsAlive() && !IsDeathPersistentSpell(aurSpellInfo) &&
        !IsDeathOnlySpell(aurSpellInfo) && !aurSpellInfo->HasAttribute(SPELL_ATTR_EX2_ALLOW_DEAD_TARGET) &&
    ...
    if ((!holder->IsPassive() && !holder->IsPersistent()) || holder->IsAreaAura())
    {
        ...
        for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second; ++iter)
        {
            ...
            if (foundHolder->GetCasterGuid() == holder->GetCasterGuid())
            {
                ...
                {
                    // can be created with >1 stack by some spellInfo mods
    ...
    // normal spellInfo or passive auras not stackable with other ranks
    ...
    if (TrackedAuraType trackedType = holder->GetTrackedAuraType())
    {
        if (Unit* caster = holder->GetCaster())             // caster not in world
        {
            ...
            switch (trackedType)
            {
                case TRACK_AURA_TYPE_SINGLE_TARGET:         // Register spellInfo holder single target
    ...
    // this can be possible it it removed indirectly by triggered spellInfo effect at ApplyModifier
    if (!holder->IsDeleted())
    {
...
bool Unit::RemoveNoStackAurasDueToAuraHolder(SpellAuraHolder* holder)
{
    ...
    // passive spellInfo special case (only non stackable with ranks)
    if (IsPassiveSpell(spellProto))
    {
    ...
    for (SpellAuraHolderMap::iterator i = m_spellAuraHolders.begin(); i != m_spellAuraHolders.end(); i = next)
    {
        ...
        // early checks that spellId is passive non stackable spellInfo
        ...
        if (IsPassiveSpell(existingSpellProto) && (spellId != existingSpellId || !spellProto->HasAttribute(SPELL_ATTR_IS_ABILITY)))
        {
            ...
            // passive non-stackable spells not stackable only with another rank of same spellInfo
        ...
        if (unique || !stackable)
        {
            // check if this spellInfo can be triggered by any talent aura
...
void Unit::RemoveAurasOnCast(uint32 flag, SpellEntry const* castedSpellEntry)
{
    for (SpellAuraHolderMap::iterator iter = m_spellAuraHolders.begin(); iter != m_spellAuraHolders.end();)
    {
        ...
        {
            if (!castedSpellEntry->HasAttribute(SPELL_ATTR_EX_ALLOW_WHILE_STEALTHED) || holder->GetSpellProto()->Dispel != DISPEL_STEALTH)
                if (!castedSpellEntry->HasAttribute(SPELL_ATTR_EX2_ALLOW_WHILE_INVISIBLE) || holder->GetSpellProto()->Dispel != DISPEL_INVISIBILITY)
...
bool Unit::HasAffectedAura(AuraType auraType, SpellEntry const* spellInfo) const
{
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (gameObj->GetSpellId())
    {
        ...
        // Need disable spellInfo use for owner
        if (createBySpell && createBySpell->HasAttribute(SPELL_ATTR_COOLDOWN_ON_EVENT))
            // note: item based cooldowns and cooldown spellInfo mods with charges ignored (unknown existing cases)
...
void Unit::RemoveGameObject(GameObject* gameObj, bool del, bool removeAura)
{
    MANGOS_ASSERT(gameObj && gameObj->GetOwnerGuid() == GetObjectGuid());

    // already removed - case for totem slots
    auto itr = std::find(m_gameObj.begin(), m_gameObj.end(), gameObj);
    if (itr == m_gameObj.end())
        return;

    gameObj->SetOwnerGuid(ObjectGuid());

    // GO created by some spellInfo
    if (uint32 spellid = gameObj->GetSpellId())
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            // Need activate spellInfo use for owner
            if (createBySpell && createBySpell->HasAttribute(SPELL_ATTR_COOLDOWN_ON_EVENT))
                // note: item based cooldowns and cooldown spellInfo mods with charges ignored (unknown existing cases)
                AddCooldown(*createBySpell);
        }
...
void Unit::SendSpellNonMeleeDamageLog(SpellNonMeleeDamage* log)
{
    ...
    data << uint8(log->periodicLog);                        // if 1, then client show spellInfo name (example: %s's ranged shot hit %s for %u school or %s suffers %u school damage from %s's spell_name
    ...
    log->attacker->SendMessageToSet(data, true);
}

void Unit::SendSpellNonMeleeDamageLog(WorldObject* attacker, Unit* target, uint32 spellID, uint32 damage, SpellSchoolMask damageSchoolMask, uint32 absorbedDamage, int32 resist, bool isPeriodic, uint32 blocked, bool criticalHit, bool split)
{
    SpellNonMeleeDamage log(attacker, target, spellID, GetFirstSchoolInMask(damageSchoolMask));
    log.damage = damage;
...
void Unit::SendSpellMiss(WorldObject* caster, Unit* target, uint32 spellID, SpellMissInfo missInfo)
{
...
void Unit::CasterHitTargetWithSpell(Unit* realCaster, Unit* target, SpellEntry const* spellInfo, bool triggered, bool success/* = true*/)
{
    ...
    else if (realCaster->CanAssist(target) && target->IsInCombat())
    {
        ...
        if (!spellInfo->HasAttribute(SPELL_ATTR_EX2_NO_INITIAL_THREAT) && !spellInfo->HasAttribute(SPELL_ATTR_EX_NO_THREAT) && realCaster->CanEnterCombat() && target->CanEnterCombat())
        {
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* calcDamageInfo) const
{
    ...
    data << uint32(0);                                      // spellInfo id, seen with heroic strike and disarm as examples.
    // HITINFO_NOACTION normally set if spellInfo
...
int32 Unit::DealHeal(Unit* pVictim, uint32 addhealth, SpellEntry const* spellInfo, bool critical)
{
...
Unit* Unit::SelectMagnetTarget(Unit* victim, Spell* spell)
{
    ...
    {
        if (spell->m_spellInfo->HasAttribute(SPELL_ATTR_IS_ABILITY) || spell->m_spellInfo->HasAttribute(SPELL_ATTR_EX_NO_REDIRECTION) || spell->m_spellInfo->HasAttribute(SPELL_ATTR_NO_IMMUNITIES))
            return nullptr;
        ...
        for (auto magnetAura : magnetAuras)
        {
            ...
            {
                if (magnet->IsAlive() && CanAttack(magnet))
                {
...
void Unit::EnergizeBySpell(Unit* victim, SpellEntry const* spellInfo, uint32 damage, Powers powerType, bool sendLog)
{
    ...
    // needs to be called after sending spellInfo log
...
/** Calculate spellInfo coefficents and level penalties for spellInfo/melee damage or heal
 *
 * this is the caster of the spellInfo/ melee attacker
 * @param spellInfo SpellEntry of the used spellInfo
 ...
 */
int32 Unit::SpellBonusWithCoeffs(SpellEntry const* spellProto, SpellEffectIndex effectIndex, int32 total, int32 benefit, int32 ap_benefit, bool donePart)
{
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellSchoolMask schoolMask, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    // Add flat bonus from spellInfo damage versus
    DoneTotal += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_FLAT_SPELL_DAMAGE_VERSUS, creatureTypeMask);

    // Add pct bonus from spellInfo damage versus
    DoneTotalMod *= GetTotalAuraMultiplierByMiscMask(SPELL_AURA_MOD_DAMAGE_DONE_VERSUS, creatureTypeMask);

    // Add flat bonus from spellInfo damage creature
    ...
    // Pets just add their bonus damage to their spellInfo damage
    // note that their spellInfo damage is just gain of their own auras
    if (GetTypeId() == TYPEID_UNIT && ((Creature*)this)->IsPet())
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellSchoolMask schoolMask, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    int32 TakenAdvertisedBenefit = SpellBaseDamageBonusTaken(GetSpellSchoolMask(spellInfo));
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    // Done fixed damage bonus auras
    int32 DoneAdvertisedBenefit = SpellBaseHealingBonusDone(GetSpellSchoolMask(spellInfo));
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellSchoolMask schoolMask, SpellEntry const* spellInfo, SpellEffectIndex effectIndex, DamageEffectType damagetype, uint32 stack, bool flat)
{
    ...
    uint32 creatureTypeMask = victim->GetCreatureTypeMask();
    schoolMask              = ((spellInfo && schoolMask != GetSpellSchoolMask(spellInfo)) ? GetSpellSchoolMask(spellInfo) : schoolMask);
    ...
    else if (isWeaponDamageBasedSpell && (APbonus || DoneFlat))
    {
        ...
        bool normalized = spellInfo ? IsSpellHaveEffect(spellInfo, SPELL_EFFECT_NORMALIZED_WEAPON_DMG) : false;
...
void Unit::ApplySpellDispelImmunity(const Aura* aura, DispelType type, bool apply)
{
    ...
    if (apply && aura->GetSpellProto()->HasAttribute(SPELL_ATTR_EX_IMMUNITY_PURGES_EFFECT))
...
bool Unit::IsVisibleForOrDetect(Unit const* u, WorldObject const* viewPoint, bool detect, bool /*inVisibleList*/, bool is3dDistance, bool spell) const
{
    ...
    if (!detect)
        return (u->GetTypeId() == TYPEID_PLAYER) ? ((Player*)u)->HasAtClient(this) : false;
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group)
{
    for (auto& i : m_Diminishing)
    {
        ...
        // If enough time has passed sinc the last spellInfo from this group was casted - reset the count
...
CharmInfo::CharmInfo(Unit* unit) :
    ...
    m_petnumber(0), m_petLastAttackCommandTime(0), m_opener(0), m_openerMinRange(0),
...
void CharmInfo::SetCharmState(UnitAI* ai, bool withNewThreatList /*= true*/)
{
    ...
    m_unitFieldBytes2_1 = m_unit->GetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_MISC_FLAGS);
    // Save unit flags
...
void CharmInfo::ResetCharmState()
{
    ...
    m_unit->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_MISC_FLAGS, m_unitFieldBytes2_1);
    // Restore unit flags
...
void CharmInfo::InitCharmCreateSpells()
{
    ...
    for (uint32 x = 0; x < CREATURE_MAX_SPELLS; ++x)
    {
        ...
        if (spellId == 2) // hardcoded spellInfo id 2 attack - only for possesss
            continue;
        ...
        else
        {
            ...
            for (uint32 i = 0; i < 3 && onlyselfcast; ++i)  // nonexistent spellInfo will not make any problems as onlyselfcast would be false -> break right away
            {
...
bool CharmInfo::AddSpellToActionBar(uint32 spellId, ActiveStates newstate, uint8 forceSlot)
{
    ...
    // new spellInfo rank can be already listed
...
bool Unit::HasAuraPetShouldAvoidBreaking(Unit* excludeCasterChannel, uint32 excludeAuraApplyMSTimeCutoff) const
{
    return HasBreakableByDamageCrowdControlAura(excludeCasterChannel, excludeAuraApplyMSTimeCutoff);
}

bool Unit::HasBreakableByDamageAuraType(AuraType type, uint32 excludeAura, uint32 excludeAuraApplyMSTimeCutoff) const
{
    AuraList const& auras = GetAurasByType(type);
    for (const auto& itr : auras)
        if (((!excludeAura || excludeAura != itr->GetSpellProto()->Id) && // Avoid self interrupt of channeled Crowd Control spells like Seduction
            (!excludeAuraApplyMSTimeCutoff || itr->GetHolder()->GetAuraApplyMSTime() >= excludeAuraApplyMSTimeCutoff)) && // Filter out CC effects which were applied before pet was last sent to attack
            (itr->GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_DAMAGE))
            return true;
    return false;
}

bool Unit::HasBreakableByDamageCrowdControlAura(Unit* excludeCasterChannel, uint32 excludeAuraApplyMSTimeCutoff) const
{
    ...
    return (HasBreakableByDamageAuraType(SPELL_AURA_MOD_CONFUSE, excludeAura, excludeAuraApplyMSTimeCutoff)
        || HasBreakableByDamageAuraType(SPELL_AURA_MOD_STUN, excludeAura, excludeAuraApplyMSTimeCutoff)
        || HasBreakableByDamageAuraType(SPELL_AURA_TRANSFORM, excludeAura, excludeAuraApplyMSTimeCutoff));
}
...
Unit* Unit::TakePossessOf(SpellEntry const* spellEntry, uint32 effIdx, float x, float y, float z, float ang)
{
    ...
    possessed->SetUInt32Value(UNIT_CREATED_BY_SPELL, spellEntry->Id);   // set the spellInfo id used to create this (may be used for removing corresponding aura
    ...
    if (player)
    {
        ...
        if (advertised)
        {
            ...
            if (!player->GetPet())
                ...
                player->PetSpellInitialize();   // reset spellInfo on pet bar
...
void Unit::InterruptSpellsCastedOnMe(bool killDelayed)
{
    ...
    for (auto& target : targets)
    {
        ...
        if (!killDelayed)
            continue;
        // 2/ Interrupt spells that are not referenced but that still have an event (like delayed spellInfo)
...
bool Unit::MeetsSelectAttackingRequirement(Unit* target, SpellEntry const* spellInfo, uint32 selectFlags, SelectAttackingTargetParams params, int32 unitConditionId) const
{
    if (selectFlags)
    {
        ...
        if ((selectFlags & SELECT_FLAG_POWER_RAGE) && target->GetPowerType() != POWER_RAGE)
            return false;

        if ((selectFlags & SELECT_FLAG_POWER_ENERGY) && target->GetPowerType() != POWER_ENERGY)
            return false;

        if ((selectFlags & SELECT_FLAG_IN_MELEE_RANGE) && !CanReachWithMeleeAttack(target))
            return false;

        if ((selectFlags & SELECT_FLAG_NOT_IN_MELEE_RANGE) && CanReachWithMeleeAttack(target))
            return false;

        if ((selectFlags & SELECT_FLAG_IN_LOS) && !IsWithinLOSInMap(target, true))
            return false;
        ...
        {
            float dist = GetDistance(target, true, DIST_CALC_COMBAT_REACH);
        ...
        if ((selectFlags & SELECT_FLAG_POWER_NOT_MANA) && target->GetPowerType() == POWER_MANA)
            return false;
    ...
    if (spellInfo)
    {
        ...
        if (selectFlags & SELECT_FLAG_NOT_IMMUNE)
            if (target->IsImmuneToSpell(spellInfo, false, GetCheckCastEffectMask(spellInfo), this))
                return false;
...
void Unit::CharmCooldownInitialize(WorldPacket& data) const
{
    // write cooldown data
    uint32 cdCount = 0;
    ...
    for (auto& cdItr : m_cooldownMap)
    {
        ...
        data << uint16(cdData->GetCategory());              // spellInfo category
